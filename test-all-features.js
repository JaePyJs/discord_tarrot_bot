...
async function testAllFeatures() {
    ...
    try {
        ...
        const majorArcana = cardUtils.allCards.filter(card => card.arcana === 'major');
        const minorArcana = cardUtils.allCards.filter(card => card.arcana === 'minor');
        ...
        suits.forEach(suit => {
            ...
            console.log(`   - ${suit.charAt(0).toUpperCase() + suit.slice(1)}: ${suitCards.length} cards`);
        });
        console.log('');
        ...
        console.log(`✅ Single Card: ${singleCard[0].name} (${singleCard[0].isReversed ? 'Reversed' : 'Upright'})`);
        ...
        console.log(`✅ Yes/No: ${yesNo[0].yesNoAnswer} (${yesNo[0].confidence} confidence)`);
        ...
        const readingTypes = ['single', 'three-card', 'celtic-cross', 'horseshoe', 'relationship', 'yes-no', 'daily', 'career'];
        ...
        const tarotCommand = require('./commands/tarot');
        ...
        console.log(`✅ Found card by exact name: ${foolCard ? foolCard.name : 'Not found'}`);
        
        const partialSearch = cardCommand.findCard('magician');
        console.log(`✅ Found card by partial name: ${partialSearch ? partialSearch.name : 'Not found'}`);
        
        const aceSearch = cardCommand.findCard('ace of cups');
        console.log(`✅ Found minor arcana: ${aceSearch ? aceSearch.name : 'Not found'}\n`);
        ...
        console.log(`✅ Achievement badges for 150 readings: ${badges.join(' ')}\n`);
        ...
        testCards.forEach(card => {
            ...
            console.log(`✅ Card ${card.id} ${card.isReversed ? 'reversed' : 'upright'}: ${confidence} confidence`);
        });
        console.log('');
