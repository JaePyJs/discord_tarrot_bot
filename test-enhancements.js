...
async function testEnhancements() {
  ...
  const progress = Math.max(
    0,
    Math.round(((testCooldown - testRemaining) / testCooldown) * 10)
  );
  const progressBar = "█".repeat(progress) + "░".repeat(10 - progress);
  console.log(
    `  ✅ Progress bar: ${progressBar} (${Math.round(
      ((testCooldown - testRemaining) / testCooldown) * 100
    )}%)`
  );
  ...
  if (testCard) {
    console.log(
      `  ✅ Enhanced card format: ${testCard.name} (${
        testCard.isReversed ? "Reversed" : "Upright"
      })`
    );
  }
